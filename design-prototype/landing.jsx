// Landing page for Totalis

const { useState: useStateL, useEffect: useEffectL, useRef: useRefL } = React;

function Landing({ navigate, tweaks, setTweak }) {
  const { headline, headlineAccent, categoryLayout } = tweaks;
  const [openFaq, setOpenFaq] = useStateL(0);

  // Scroll to hash if present in window
  useEffectL(() => {
    if (window.__scrollTarget) {
      const id = window.__scrollTarget;
  ...
  return (
    <main>
      ...
      <section className="section" style={{ paddingTop: 80, paddingBottom: 60, position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="hero-shield-bg"></div>
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: 820 }}>
            ...
            <h1 className="display fade-up" style={{ fontSize: 'clamp(40px, 6.5vw, 84px)', margin: '20px 0 0' }}>
              {headline.split('{accent}')[0]}<span className="accent">{headlineAccent}</span>{headline.split('{accent}')[1]}
            </h1>
            ...
            <div className="fade-up" style={{ display: 'flex', gap: 12, marginTop: 36, flexWrap: 'wrap' }}>
              <button className="btn btn-primary btn-lg" onClick={() => document.getElementById('pacotes').scrollIntoView({ behavior: 'smooth' })}>
                Ver pacotes <Icon.ArrowRight width="18" height="18" />
              </button>
              <button className="btn btn-ghost btn-lg" onClick={() => document.getElementById('como-funciona').scrollIntoView({ behavior: 'smooth' })}>
                Como funciona?
              </button>
            </div>
            <div className="fade-up" style={{ display: 'flex', gap: 28, marginTop: 44, flexWrap: 'wrap', color: 'var(--fg-faint)', fontSize: 13 }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <Icon.Check width="16" height="16" style={{ color: 'var(--success)' }} /> Em 20 dias úteis
              </span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <Icon.Check width="16" height="16" style={{ color: 'var(--success)' }} /> Lei 9.610/98
              </span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <Icon.Check width="16" height="16" style={{ color: 'var(--success)' }} /> Pix, cartão, boleto
              </span>
            </div>
          </div>
      ...
      <section className="section" id="categorias" style={{ paddingTop: 40 }}>
        <div className="container">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end', marginBottom: 40, flexWrap: 'wrap', gap: 20 }}>
            ...
            <p className="text-muted" style={{ maxWidth: 380, fontSize: 15, margin: 0 }}>
              Da música ao código, do roteiro à planta arquitetônica. Se você criou, a gente registra.
            </p>
          </div>

          <div className={`cat-${categoryLayout}`}>
            {CATEGORIES.map((cat) => {
              const IconComp = Icon[cat.icon] || Icon.Text;
              return (
                <div key={cat.key} className="cat-card" onClick={() => { document.getElementById('pacotes').scrollIntoView({ behavior: 'smooth' }); }}>
                  <div className="icon-wrap"><IconComp /></div>
                  <div className="cat-name">{cat.name}</div>
                  <div className="cat-desc">{cat.desc}</div>
                  <div className="cat-arrow"><Icon.ArrowRight width="16" height="16" /></div>
                </div>
              );
            })}
          </div>
      ...
      <section className="section">
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 18 }}>
            {BENEFICIOS.map((b, i) => {
              const IconComp = Icon[b.icon] || Icon.Check;
              return (
                <div key={i} className="card">
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                    ...
                    }}><IconComp width="18" height="18" /></div>
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10, flex: 1 }}>
                  {[
                    ...
                    <li key={i} style={{ display: 'flex', gap: 10, fontSize: 14, color: 'var(--fg-muted)' }}>
                      <Icon.Check width="16" height="16" style={{ color: 'var(--success)', flexShrink: 0, marginTop: 3 }} />
                      <span>{f}</span>
                    </li>
                  ))}
                </ul>
                <button className={`btn ${p.featured ? 'btn-primary' : 'btn-ghost'} btn-lg`} style={{ width: '100%', justifyContent: 'center' }}
                        onClick={() => navigate('/obrigado', null, { pacote: p })}>
                  Comprar agora <Icon.ArrowRight width="16" height="16" />
                </button>
              </div>
            ))}
          </div>
            ...
            {FAQ.map((f, i) => (
              <div key={i} className={`faq-item ${openFaq === i ? 'open' : ''}`}>
                <div className="faq-q" onClick={() => setOpenFaq(openFaq === i ? -1 : i)}>
                  <span>{f.q}</span>
                  <Icon.Plus className="chev" width="20" height="20" />
                </div>
        ...
        <div className="container">
          <div style={{
            ...
            <div style={{ position: 'relative' }}>
              ...
              <button className="btn btn-primary btn-lg" onClick={() => document.getElementById('pacotes').scrollIntoView({ behavior: 'smooth' })}>
                Começar agora <Icon.ArrowRight width="18" height="18" />
              </button>
            </div>
          </div>
        </div>
